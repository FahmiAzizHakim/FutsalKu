...
class Lapangan extends MY_Controller {
	...
	public function add()
	{
		...
		$field_type = json_decode(($this->curl->simple_get($this->API.'Kontak/code_bycategory', $param_fieldtype)), true);

		$param_fieldroom = array('code_category' => 'FLR');
		$field_room = json_decode(($this->curl->simple_get($this->API.'Kontak/code_bycategory', $param_fieldroom)), true);

		$param_day = array('code_category' => 'DYT');
		$day_type = json_decode(($this->curl->simple_get($this->API.'Kontak/code_bycategory', $param_day)), true);
